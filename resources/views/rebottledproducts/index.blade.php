...
<table class="table table-striped">
        <tr>
            ...
            <th>Number of bottle/s produce</th>
            <th>Designation</th>
            <th class="text-left">
                <a href="{{ route('rebottledproducts.create') }}" class="btn btn-success btn-small">
                    <i class="glyphicon glyphicon-plus"></i>
                </a>
            </th>
        </tr>

        @foreach($rebottled_product as $key => $value)
        <tr>
            <td style="width: 100px;">{{ $value->id }}</td>
            <td>{{ $value->product_id }}</td>
            <td>{{ $value->use_product }}</td>
